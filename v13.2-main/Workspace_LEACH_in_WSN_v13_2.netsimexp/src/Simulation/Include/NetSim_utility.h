...
#pragma region FILEENCRYPTION
	...
	_Check_return_
		NETSIM_UTILITY_EXPORT ptrFILEENCRYPTIONHANDLE __cdecl open_encrypted_file(
			_In_z_ char const* _FileName
		);
	
	_Success_(return != -1)
		_Check_return_opt_
		NETSIM_UTILITY_EXPORT int __cdecl close_encrypted_file(
			_In_ _Post_invalid_ ptrFILEENCRYPTIONHANDLE _Stream
		);
	
	_Success_(return == _Buffer)
		_Check_return_opt_
		NETSIM_UTILITY_EXPORT char* __cdecl read_line_from_encrypted_file(
	...
	_Success_(return == 0)
		_Check_return_opt_
		NETSIM_UTILITY_EXPORT int encrypt_file(
